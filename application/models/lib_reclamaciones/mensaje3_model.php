...
class Mensaje3_model extends CI_Model {
...
//                                $this->getCDmeOficina(),
    ...
    function mensajelistar_reclamo3(){
    ...
    function mensajelistar_sugerencia3(){
        $query = $this->bdlib_reclamaciones->query("select m.nMenIdMensaje as id, CONCAT(p.apecol,'',p.apematcol,', ',p.nomcol) as emisor,m.cMenAsunto as asunto, m.dMenFecha as fecha,m.cMenEstado as estado
        from tb_lib_rec_mensaje1 m 
        inner join bdcolegio.colegiado p on m.nPerIdPersona = p.regcol 
        where p.regcol=142180
    ...
    function mensajelistar_opiniones3(){
        $query = $this->bdlib_reclamaciones->query("select m.nMenIdMensaje as id, CONCAT(p.apecol,'',p.apematcol,', ',p.nomcol) as emisor,m.cMenAsunto as asunto, m.dMenFecha as fecha,m.cMenEstado as estado
        from tb_lib_rec_mensaje1 m 
        inner join bdcolegio.colegiado p on m.nPerIdPersona = p.regcol 
        where p.regcol=142180
...
//    function noticiasempresarialqry($criterio='') {
//        $query = $this->db->query("CALL USP_GEN_S_NOTICIAS2 (?)", $criterio['criterio']);
//        if ($query->num_rows() > 0) {
//            $this->db->close();
//            return $query->result_array();
//        } else {
