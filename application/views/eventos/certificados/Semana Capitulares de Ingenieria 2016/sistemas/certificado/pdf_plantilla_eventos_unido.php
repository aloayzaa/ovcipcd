     ...
     <p id="certificado" style="line-height:12px;"><strong>CERTIFICADO </strong></p>
          ...
          <?php if(strlen($certificado['nombres'][$i])<=40) {?>
             ...
             <?php if(strlen($certificado['titulo_evento'][$i])<=40 ||  $certificado['horas'][$i]!=0){?>
              <p id="contenido">Por su participación como <strong><?php echo  $certificado['tipocertificado'][$i] ?></strong> <?php echo $conjuntor; ?> <strong><?php echo $certificado['titulo_evento'][$i];?></strong>, ofrecida
               en el marco de la Semana del Capítulo de Ingeniería de Sistemas, Computación e Informática del Consejo Departamental de La Libertad,
             <?php  if ( $certificado['horas'][$i]!=0){?>
                    con una duración de <?php echo  $certificado['horas'][$i]  ?> hora académica.
          
             <?php } }          
             
                    else { ?>
                    <p id="contenido1">Por su participación como <strong><?php echo  $certificado['tipocertificado'][$i] ?></strong> <?php echo $conjuntor; ?> <strong><?php echo $certificado['titulo_evento'][$i];?></strong>, ofrecida 
                      en el marco de la Semana del Capítulo de Ingeniería de Sistemas, Computación e Informática del Consejo Departamental de La Libertad,  
             <?php  if ( $certificado['horas'][$i]!=0){?>
                    con una duración de <?php echo  $certificado['horas'][$i]  ?> hora académica.
             <?php } }?>
