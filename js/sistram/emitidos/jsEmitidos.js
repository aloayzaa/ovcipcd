$(document).ready(function() {  
    ...
    $('#Tabs_Decanato').tabs();  //convierte a tabs 
      ...
      get_page('emitidos/load_listar_view_emitidos/'+anio,'qry_emitidos');
    
    
  
     $('#EmitidosListar').bind('click', function(){
        msgLoading2("#qry_emitidos");
        var anio=$("#listadeanios").val();
        get_page('emitidos/load_listar_view_emitidos/'+anio,'qry_emitidos');
    });
...
function expedienteDetalle(expediente){
    $("#div_detalle_emitidos").hide("slow");
      var rdn=Math.random()*11;
        $.post('emitidos/expedienteDetalle/'+expediente,{
            rdn:rdn
        }, function(data){
